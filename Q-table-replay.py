...
config = {"env": args.env, "lr": args.lr, "lr_decay": args.lr_decay, "y": args.y, "eps": args.eps, "eps_decay": args.eps_decay, "min_eps": args.min_eps, "cost": args.cost}
...
class QTable:
    def __init__(self, env, lr=args.lr, lr_decay=args.lr_decay, y=args.y, eps=args.eps, eps_decay=args.eps_decay, min_eps=args.min_eps, train_size=5241):
        self.env = env
    ...
    def choose_action(self, q_row, actions_remain):
        ...
        if np.random.random() < self.eps:
            return np.random.choice(actions_remain)
        else:
    ...
    def train(self): 
        ...
        for ep in tqdm(range(self.num_episodes)):
            ...
            while True:
                ...
                action = self.choose_action(self.q_table[s], actions_remain)
                ...
                if not d:
                    self.q_table[s][action] += self.lr*(r + self.y*np.max(self.q_table[s1]) - self.q_table[s][action]) 
                else: 
                    #If done then update q_value without future rewards 
                    self.q_table[s][action] += self.lr*(r - self.q_table[s][action]) 
